...
describe( `${appName} under monkeys`, function() {
    ...
    it(`visits ${appName} and survives monkeys`, function() {
        ...
        if(pcg === 100){
            ...
            cy.wait(1000)
            cy.get('form').within(() => {
                cy.get('input[name="identification"]').type('c.barreiroh@uniandes.edu.co')
                cy.get('input[name="password"]').type('Q123456789')
                cy.get('button[id="ember10"]').click()
            });
            cy.wait(1000)
            cy.visit('http://localhost:2370/ghost/#/editor/page/6368380e90a5ccc293bc617a')
            /* cy.visit("#/pages/") */
